...
public class IfAny implements Command {
	...
	/**
	 * * Executes this <code>IfAny</code> command. If the evaluation of any comparison gives true as a result nothing is done since the PC has already been incremented by the act method in <code>Ship</code>.
	 * If the evaluation of all comparisons gives false as a result the ship's program counter is set to elsePC.
	...
	/**
	* Overrides the equals method of Object. An Object is equal to an <code>IfAny</code> if the object
	 * is an <code>IfAny</code> as well and if the elsePC of both <code>IfAny</code>s are the same. Additionally the lists of comparisons clauses have to be equal.
